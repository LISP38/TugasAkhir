class RekapSatkerModel {
  ...
  factory RekapSatkerModel.fromMap(Map<String, dynamic> map) {
    return RekapSatkerModel(
      ...
      kuotaAwal: (map['kuota_awal'] is num) ? (map['kuota_awal'] as num).toDouble() : double.tryParse(map['kuota_awal']?.toString() ?? '0') ?? 0.0,
      kuotaTerpakai: (map['kuota_terpakai'] is num) ? (map['kuota_terpakai'] as num).toDouble() : double.tryParse(map['kuota_terpakai']?.toString() ?? '0') ?? 0.0,
    );
  }
}
